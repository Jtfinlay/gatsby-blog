...
![Picture of 'Fiddler' splash](/media/csharp-xaml-webview-navigate-timeoutfiddler.PNG)
...
![Picture of Fiddler HTTPS settings](/media/csharp-xaml-webview-navigate-timeoutfiddler-settings.PNG)
...
![Picture of Fiddler auto-responder settings](/media/csharp-xaml-webview-navigate-timeoutfiddler-autoresponder.PNG)
...
Great, things are initialized. There is one more piece we need to think through however - the _retry_. We are not always navigating to a `Uri`, but we could be using a `HttpRequestMessage`, or any other navigation types as well. We need a way to generalize the method call so we can recall it in case of failure. Using an [`Action delegate`](<https://msdn.microsoft.com/en-us/library/018hxwa8(v=vs.110).aspx>) will help us with this.
